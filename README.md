...
After a managed PPO run completes, the sampled training preview can stay open
while Newton releases the replicated training batch. Checkpoint replay then
uses a one-arm environment and the same read-only viewer contract. PPO training
and replay can select either the default Viser provider or the optional OVRTX
provider.
...
artifact export. Its preview renders one sampled training environment while
the full batch continues stepping on the training device. Viser supplies the
target marker, end-effector trail, metrics, and environment selector; OVRTX
