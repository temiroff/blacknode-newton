...
class RosbridgeSession:
    ...
    def _on_command(self, message: dict[str, Any]) -> None:
        try:
            ...
            positions = {str(name): float(value) for name, value in zip(names, values)}
            if not all(math.isfinite(value) for value in positions.values()):
                raise ValueError("JointState command contains non-finite positions")
            runtime.command_session(self.run_id, positions, source="rosbridge")
            self.received += 1
    ...
    def _publish_loop(self) -> None:
        ...
        while not self.stop_event.wait(period):
            try:
                ...
                if (
                    status.get("armed")
                    and status.get("last_command_source") == "rosbridge"
...
def _status(bridge_id: str) -> dict[str, Any]:
    ...
    if bridge is None:
        return {
            ...
            "url": "", "published": 0, "received": 0, "rejected": 0, "last_error": "",
        }
...
@node(
    ...
    live=True,
    category=_CATEGORY,
    description="Publish Newton articulation state and receive armed joint targets as ROS 2 JointState messages over rosbridge WebSocket.",
    inputs={
        "trigger": AnyPort,
...
def newton_rosbridge(ctx: dict) -> dict:
    ...
    try:
            ...
            with _LOCK:
                ...
                if prior is not None and prior.status().get("running"):
                    ...
                    bridge = RosbridgeSession(
                        bridge_id,
                        str(ctx.get("run_id") or "newton-scene"),
        ...
        return {
            "ok": False, "running": False, "connected": False, "bridge": _status(bridge_id),
            "report": f"Newton ROS bridge FAILED: {type(exc).__name__}: {exc}",
        }
